...
// `fill="currentColor"` inherits from the `text-brand` color, so recoloring the
// brand is a one-line change to the `--color-brand` token in globals.css.
...
export default function PulseMark({ className }: { className?: string }) {
  return (
    <svg
      ...
      fill="currentColor"
      aria-hidden="true"
      className={cn("block h-10 w-auto text-brand", className)}
    >
      <path d={PULSE_PATH} />
    </svg>
  );
